...
function [sys,x0,str,ts] = fRBFgausspeed1optimal(t,x,u,flag,PasoGrado,numRBFs,sigma,limitinf,limitsup,numfig,ver)
persistent RBF1vs
...
switch flag,
  ...
  %%%%%%%%%%%%%%%%%%
  case 0,
    [sys,x0,str,ts,RBF1vs]=mdlInitializeSizes(PasoGrado,numRBFs,limitinf,limitsup,sigma);
  ...
  case 3,
      ...
      [sys]=mdlOutputs(t,x,u,PasoGrado,numRBFs,limitinf,limitsup,numfig,ver,RBF1vs);
...
%=============================================================================
%
function [sys,x0,str,ts,RBF1vs]=mdlInitializeSizes(PasoGrado,numRBFs,limitinf,limitsup,sigma)
...
%center displacement
  ...
  % for i=1:numRBFs,
         ...
         % error(i) = a + (b-a) * rand(1);
              ...
              % error(i)=5+20*randn(1);
   % end

               


%RBFs=RBF(x,numeroRBF,sigma,ERROR3); no fast code
...
%=============================================================================
%
function [sys]=mdlOutputs(t,x,u,PasoGrado,numRBFs,limitinf,limitsup,numfig,ver,RBF1vs)
%fast velocity RBFs
%angle vector
...
entrada= u(1);
Ibase=0.1;
len=length(RBF1vs(1,:))-1;
indice=((entrada-limitinf)/(limitsup-limitinf))*len + 1;
sys=RBF1vs(:,round(indice))+Ibase;

if ver==1
    x=limitinf:PasoGrado:limitsup
    subplot(7,2,numfig+7)
    plot(x,RBF1vs(1:numRBFs,:),'b')
    title('Velocidad Articular')
    hold on
    plot(entrada,RBF1vs(:,round(indice))+Ibase,'kx')
    hold off
end
% end mdlOutputs
